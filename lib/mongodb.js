...
MongoDB.prototype.create = function (model, data, options, callback) {
	...
	this.execute(model, 'insert', data, {
		safe: true
	}, function (err, result) {
		if (self.debug) {
...
MongoDB.prototype.replaceWithOptions = function (model, id, data, options, cb) {
	...
	var idName = self.idName(model);
	delete data[idName];
	this.execute(model, 'update', {
		_id: id
	}, data, options, function (err, info) {
		if (this.debug) debug('updateWithOptions.callback', model, {
			_id: id
		}, data, err, info);
...
MongoDB.prototype.update =
	MongoDB.prototype.updateAll = function updateAll(model, where, data, options, cb) {
		...
		this.execute(model, 'update', where, data, {
				multi: true,
				upsert: false
			},
			function (err, info) {
